...
<h1 class="text-center mt-3 mb-4">Food products</h1>
    ...
    <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="product-card">
                <img src="<?php echo htmlspecialchars($row['food_image']); ?>" alt="Food Image">
                <h3><?php echo htmlspecialchars($row['food_name']); ?></h3>
                <p>Food Price: $<?php echo number_format($row['food_price'], 2); ?></p>
                <p>Delivery Price: $<?php echo number_format($row['delivery_price'], 2); ?></p>
                <button onclick='openModal(<?php echo json_encode($row, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)'>Order Now</button>
...
<div class="d-sm-flex justify-content-between py-1">
    ...
    <div class="modal">
        <h2 id="modal-title">Order Item</h2>
        <p>Food Price: <span id="modal-food-price"></span></p>
        <p>Delivery Price: <span id="modal-delivery-price"></span></p>
        <div>
    ...
    function openModal(item) {
        currentItem = item;
        document.getElementById('modal-title').innerText = item.food_name;
        document.getElementById('modal-food-price').innerText = `$${item.food_price}`;
        document.getElementById('modal-delivery-price').innerText = `$${item.delivery_price}`;
    ...
    function updateQuantity(change) {
        ...
        let quantity = parseInt(quantityEl.innerText) + change;
    ...
    function addToCart() {
        ...
        $.post('update-cart.php', {
            food_id: currentItem.id,
            food_name: currentItem.food_name,
            food_price: currentItem.food_price,
            delivery_price: currentItem.delivery_price,
            quantity: quantity,
            action: 'add'
        }, function(data) {
            ...
            console.error('Error adding to cart:', error); // Debugging: Log any AJAX errors
        });
    }
</script>
